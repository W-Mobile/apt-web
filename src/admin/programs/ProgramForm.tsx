      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          <div className="border-t border-stone-700/50 pt-6 mb-4">
            <h3 className="text-base font-medium text-stone-200">Uppvärmning</h3>
            <p className="text-xs text-stone-500 mt-1">Gäller för alla perioder i programmet.</p>
          </div>
          <div className="space-y-2">
            {warmupWorkoutID && (
              <div className="flex items-center justify-between gap-2 text-sm py-0.5 pl-1">
                <span className="text-stone-300">
                  {availableWorkouts.find((w) => w.id === warmupWorkoutID)?.name ?? '...'}
                </span>
                <button type="button" onClick={() => setWarmupWorkoutID(null)}
            ...
            <div className="mt-3">
              <SearchableSelect
                ...
                selectedIds={new Set(warmupWorkoutID ? [warmupWorkoutID] : [])}
                onSelect={(id) => setWarmupWorkoutID(id)}
                getId={(w) => w.id}
                getLabel={(w) => w.name}
                placeholder={warmupWorkoutID ? 'Byt uppvärmnings-workout...' : 'Sök och välj uppvärmnings-workout...'}
              />
            </div>
          </div>
        </div>
